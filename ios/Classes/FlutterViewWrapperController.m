...
-(void)viewDidAppear:(BOOL)animated {
    ...
    if(self.curFlutterRouteName.length && self.lastSnapshot){
        [[HybridStackPlugin sharedInstance].methodChannel invokeMethod:@"popToRouteNamed" arguments:self.curFlutterRouteName];
    }
...
- (void)viewDidDisappear:(BOOL)animated {
    ...
    if(!ary.count){
        [[HybridStackPlugin sharedInstance].methodChannel invokeMethod:@"popToRoot" arguments:nil];
    }

    NSArray *curStackAry = rootNav.viewControllers;
    NSInteger idx = [curStackAry indexOfObject:self];
    if(idx == NSNotFound){
        [[HybridStackPlugin sharedInstance].methodChannel invokeMethod:@"popRouteNamed" arguments:self.lastFlutterRouteName];
    }
}
